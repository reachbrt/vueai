...
describe('useChatEngine', () => {
  ...
  it('should load messages from localStorage if persistenceKey is provided', () => {
    ...
    // We need to mock the localStorage.getItem before creating the composable
    ...
    // Since we're mocking onMounted to execute immediately, we need to check the messages directly
  ...
  it('should limit the number of messages if maxMessages is provided', async () => {
    // Mock the chat method to return a consistent response
    const mockChat = vi.fn().mockResolvedValue('This is a mock response');
    const mockClient = {
      chat: mockChat,
      chatStream: vi.fn()
    };
    ...
    const { messages, sendMessage } = useChatEngine({
      ...
      maxMessages: 2,
      streaming: false // Important: use non-streaming mode to use the chat method
    });
