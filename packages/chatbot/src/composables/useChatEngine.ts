...
export function useChatEngine(options: ChatOptions): ChatEngineReturn {
  ...
  const sendMessage = async (content: string): Promise<void> => {
    ...
    try {
      console.log('ChatEngine - Sending message with client:', client);
      console.log('ChatEngine - API Key available:', !!options.apiKey);
      ...
      console.log('ChatEngine - Messages to send:', messagesToSend);
      ...
      if (streaming) {
        ...
        if (useProxy) {
          ...
          console.log('ChatEngine - Using client directly for streaming chat');
        ...
        if (useProxy) {
          ...
          // Use the client directly
          console.log('ChatEngine - Using client directly for non-streaming chat');
          response = await client.chat(messagesToSend);
          console.log('ChatEngine - Received response:', response);
        }
